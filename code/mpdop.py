...
# ---- PATH/PROF/BLND EDIT OPS DRAW-FUNCTION
...
def torus(cns, cvs, pgob):
    ...
    rc.prop(pgob, "tor_dim", index=0, text="")
    rc = col.row(align=True)
    rc.prop(pgob, "tor_dim", index=1, text="")
    row = cvs.row(align=True)
    row.prop(pgob, "tor_stp", text="")
    row.prop(pgob, "tor_pha", text="")
...
# ---- PATH/PROF/CU_LOC-ROT, BLND, PNLOC, CUDEP, PNRAD EDIT OPS PARAMS DRAW-FUNCTION


def params_layout_draw(box, pgob, names, shuffle):
    ...
    for n in names:
        ...
        row.label(text=n)
    if shuffle:
        names = ("Lerp", "Style", "Random", "Shuffle")
    else:
        names = ("Lerp", "Style", "Random")
    for n in names:
        row = sc.row()
        row.label(text=n)
    sc = s.column(align=True)
    ...
    row = col.row(align=True)
    rc = row.column(align=True)
    rc.prop(pgob, "rnduse", text="on" if pgob.rnduse else "off", toggle=True)
    rc = row.column(align=True)
    rc.enabled = pgob.rnduse
    rc.prop(pgob, "rndval", text="")
    rc = row.column(align=True)
    rc.enabled = pgob.rnduse
    rc.prop(pgob, "rndseed", text="")
    if shuffle:
        row = col.row(align=True)
        row.enabled = pgob.rnduse
        row.prop(pgob, "rndshuff", text="on" if pgob.rndshuff else "off", toggle=True)
