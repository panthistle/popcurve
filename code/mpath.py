...
class Helix:
    ...
    def get_locs(self):
        ...
        height = -self.length
        base = -height / 2
        if not self.hlerp or (self.exp == 1.0):
...
class Torus:
    """torus (XYZ, counterclock from x=dim[0]/2, y=0, z=0): path"""
    ...
    def update(self, dct):
        self.npts = dct["res_tor"]
        self.dim = dct["tor_dim"][:]
        self.steps = dct["tor_stp"]
        self.pha = dct["tor_pha"]

    def anim_update(self, *args):
        self.dim, self.steps, self.pha = args

    def get_locs(self):
        npts = self.npts
        r1, r2 = (self.dim[0] / 2, self.dim[1] / 2)
        dt = 2 * math.pi / npts
        tls = [dt * i for i in range(npts)]
        n = self.steps
        p = self.pha
        rls = [r1 + r2 * math.cos(n * t + p) for t in tls]
        return [
            Vector((r3 * math.cos(t), r3 * math.sin(t), r2 * math.sin(n * t + p)))
            for r3, t in zip(rls, tls)
        ]
...
class Custom:
    ...
    def update(self, dct):
        ...
        self._udim = dct["user_dim"][:]
        self.dim = dct["cust_dim"][:]
